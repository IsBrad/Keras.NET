...
namespace Keras.PreProcessing
{
    ...
    public class ImageDataGenerator : Base
    {
        ...
        /// <param name="zoom_range">
        ///     Float or [lower, upper]. Range for random zoom. If a float, [lower, upper] = [1-zoom_range,
        ///     1+zoom_range].
        /// </param>
        /// <param name="channel_shift_range"> Float. Range for random channel shifts.</param>
        /// <param name="fill_mode">
        ///     One of {"constant", "nearest", "reflect" or "wrap"}. Default is 'nearest'. Points outside the
        ///     boundaries of the input are filled according to the given mode, 'constant'-> kkkkkkkk|abcd|kkkkkkkk (cval=k),
        ///     'nearest'-> aaaaaaaa|abcd|dddddddd, 'reflect'-> abcddcba|abcd|dcbaabcd, 'wrap'-> abcdabcd|abcd|abcdabcd
        /// </param>
        ...
        /// <param name="rescale">
        ///     rescaling factor. Defaults to None. If None or 0, no rescaling is applied, otherwise we multiply
        ///     the data by the value provided (after applying all other transformations).
        /// </param>
        /// <param name="preprocessing_function">
        ///     function that will be implied on each input. The function will run after the
        ///     image is resized and augmented. The function should take one argument: one image (Numpy tensor with rank 3), and
        ///     should output a Numpy tensor with the same shape.
        /// </param>
        /// <param name="data_format">
        ///     Image data format, either "channels_first" or "channels_last". "channels_last" mode means
        ///     that the images should have shape  (samples, height, width, channels), "channels_first" mode means that the images
        ///     should have shape  (samples, channels, height, width). It defaults to the image_data_format value found in your
        ...
        public ImageDataGenerator(bool featurewise_center = false, bool samplewise_center = false,
            bool featurewise_std_normalization = false,
            bool samplewise_std_normalization = false, bool zca_whitening = false, float zca_epsilon = 1e-06f,
            int rotation_range = 0,
            float width_shift_range = 0.0f, float height_shift_range = 0.0f, float[] brightness_range = null,
            float shear_range = 0.0f,
            float zoom_range = 0.0f, float channel_shift_range = 0.0f, string fill_mode = "nearest", float cval = 0.0f,
            bool horizontal_flip = false, bool vertical_flip = false, float? rescale = null,
        ...
        /// <param name="transform_parameters">
        ///     Dictionary with string - parameter pairs describing the transformation. Currently, the following parameters from
        ///     the dictionary are used:
        ///     <list type="bullet">
        ///         <item>'theta': Float.Rotation angle in degrees.</item><item>'tx': Float.Shift in the x direction.</item>
        ///         <item>'ty': Float.Shift in the y direction.</item><item>'shear': Float.Shear angle in degrees.</item>
        ///         <item>'zx': Float.Zoom in the x direction.</item><item>'zy': Float.Zoom in the y direction.</item>
        ///         <item>'flip_horizontal': Boolean.Horizontal flip.</item><item>'flip_vertical': Boolean.Vertical flip.</item>
        ///         <item>'channel_shift_intencity': Float.Channel shift intensity.</item>
        ///         <item>'brightness': Float.Brightness shift intensity.</item>
        ///     </list>
        /// </param>
        ...
        ///     This computes the internal data stats related to the data-dependent transformations, based on an array of sample
        ///     data.
        ...
        /// </summary>
        /// <param name="x">
        ///     Input data. Numpy array of rank 4 or a tuple. If tuple, the first element should contain the images
        ///     and the second element another numpy array or a list of numpy arrays that gets passed to the output without any
        ///     modifications. Can be used to feed the model miscellaneous data along with the images. In case of grayscale data,
        ///     the channels axis of the image array should have value 1, in case of RGB data, it should have value 3, and in case
        ...
        /// <param name="save_to_dir">
        ///     None or str (default: None). This allows you to optionally specify a directory to which to
        ///     save the augmented pictures being generated (useful for visualizing what you are doing).
        /// </param>
        /// <param name="save_prefix">
        ///     Str (default: ''). Prefix to use for filenames of saved pictures (only relevant if
        ///     save_to_dir is set).
        /// </param>
        ...
        ///     input) or a list of numpy arrays (in the case with additional inputs) and y is a numpy array of corresponding
        ///     labels. If 'sample_weight' is not None, the yielded tuples are of the form (x, y, sample_weight). If y is None,
        ///     only the numpy array x is returned.
        /// </returns>
        public KerasIterator Flow(NDarray x, NDarray y = null, int batch_size = 32, bool shuffle = true,
            NDarray sample_weight = null, int? seed = null,
        ...
        ///     string, path to the target directory. It should contain one subdirectory per class. Any PNG,
        ///     JPG, BMP, PPM or TIF images inside each of the subdirectories directory tree will be included in the generator. See
        ///     this script for more details.
        /// </param>
        /// <param name="target_size">
        ///     Tuple of integers (height, width), default: (256, 256). The dimensions to which all images
        ///     found will be resized.
        /// </param>
        /// <param name="color_mode">
        ///     One of "grayscale", "rgb", "rgba". Default: "rgb". Whether the images will be converted to
        ///     have 1, 3, or 4 channels.
        /// </param>
        /// <param name="classes">
        ///     Optional list of class subdirectories (e.g. ['dogs', 'cats']). Default: None. If not provided,
        ///     the list of classes will be automatically inferred from the subdirectory names/structure under directory, where
        ///     each subdirectory will be treated as a different class (and the order of the classes, which will map to the label
        ///     indices, will be alphanumeric). The dictionary containing the mapping from class names to class indices can be
        ///     obtained via the attribute class_indices.
        /// </param>
        /// <param name="class_mode">
        ///     One of "categorical", "binary", "sparse", "input", or None. Default: "categorical".
        ///     Determines the type of label arrays that are returned:
        /// </param>
        /// <param name="batch_size"> Size of the batches of data (default: 32).</param>
        /// <param name="shuffle">
        ///     Whether to shuffle the data (default: True) If set to False, sorts the data in alphanumeric
        ///     order.
        /// </param>
        /// <param name="seed"> Optional random seed for shuffling and transformations.</param>
        /// <param name="save_to_dir">
        ...
        ///     Interpolation method used to resample the image if the target size is different from that
        ///     of the loaded image. Supported methods are "nearest", "bilinear", and "bicubic". If PIL version 1.1.3 or newer is
        ///     installed, "lanczos" is also supported. If PIL version 3.4.0 or newer is installed, "box" and "hamming" are also
        ///     supported. By default, "nearest" is used.
        /// </param>
        /// <returns>
        ///     A DirectoryIterator yielding tuples of (x, y) where x is a numpy array containing a batch of images with shape
        ///     (batch_size, *target_size, channels) and y is a numpy array of corresponding labels.
        /// </returns>
        public KerasIterator FlowFromDirectory(string directory, Tuple<int, int> target_size = null,
            string color_mode = "rgb", string[] classes = null, string class_mode = "categorical",
            int batch_size = 32, bool shuffle = true, int? seed = null, string save_to_dir = "",
            string save_prefix = "", string save_format = "png",
            bool follow_links = false, string subset = "", string interpolation = "nearest")
        {
            var parameters = new Dictionary<string, object>();
            parameters["directory"] = directory;
            parameters["target_size"] = target_size == null
                ? new Shape(256, 256)
                : new Shape(target_size.Item1, target_size.Item2);
            parameters["color_mode"] = color_mode;
            parameters["classes"] = classes;
            parameters["class_mode"] = class_mode;
            parameters["batch_size"] = batch_size;
