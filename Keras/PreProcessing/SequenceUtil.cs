...
namespace Keras.PreProcessing
{
    ...
    {
        private static readonly dynamic caller = Instance.keras.preprocessing.sequence;
        ...
        ///     Sequences longer than num_timesteps are truncated so that they fit the desired length.The position where padding or
        ///     truncation happens is determined by the arguments padding and truncating, respectively.
        ...
        public static NDarray PadSequences(NDarray sequences, int? maxlen = null, string dtype = "int32",
            string padding = "pre", string truncating = "pre", float value = 0)
        {
        ...
        /// </summary>
        /// <param name="sequence">
        ///     A word sequence (sentence), encoded as a list of word indices (integers). If using a
        ///     sampling_table, word indices are expected to match the rank of the words in a reference dataset (e.g. 10 would
        ///     encode the 10-th most frequently occurring token). Note that index 0 is expected to be a non-word and will be
        ///     skipped.
        /// </param>
        /// <param name="vocabulary_size"> Int, maximum possible word index + 1</param>
        /// <param name="window_size">
        ///     Int, size of sampling windows (technically half-window). The window of a word w_i will be [i
        ///     - window_size, i + window_size+1].
        /// </param>
        /// <param name="negative_samples">
        ///     Float >= 0. 0 for no negative (i.e. random) samples. 1 for same number as positive
        ///     samples.
        /// </param>
        /// <param name="shuffle"> Whether to shuffle the word couples before returning them.</param>
        /// <param name="categorical">
        ///     bool. if False, labels will be integers (eg. [0, 1, 1 .. ]), if True, labels will be
        ///     categorical, e.g. [[1,0],[0,1],[0,1] .. ].
        /// </param>
        /// <param name="sampling_table">
        ///     1D array of size vocabulary_size where the entry i encodes the probability to sample a
        ///     word of rank i.
        /// </param>
        /// <param name="seed"> Random seed.</param>
        /// <returns>couples, labels: where couples are int pairs and  labels are either 0 or 1.</returns>
        public static NDarray SkipGrams(NDarray sequence, int vocabulary_size, int window_size = 4,
            float negative_samples = 1.0f, bool shuffle = true,
        ...
        ///     Used for generating the sampling_table argument for skipgrams.sampling_table[i] is the probability of sampling the
        ///     word i-th most common word in a dataset(more common words should be sampled less frequently, for balance).
