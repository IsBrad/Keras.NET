...
namespace Keras.Layers.Activation
{
    ...
    public class Activations : Base
    {
        private static readonly dynamic caller = Instance.keras.activations;
        ...
        {
            var parameters = new Dictionary<string, object> {["x"] = x, ["axis"] = axis};
            return new NDarray(InvokeStaticMethod(caller, "softmax", parameters));
        }

        /// <summary>
        ///     Rectified Linear Unit. With default values, it returns element-wise max(x, 0). Otherwise, it follows: f(x) =
        ///     max_value for x >= max_value, f(x) = x for threshold <= x<max_value, f( x)= alpha* ( x - threshold) otherwise.
        /// </summary>
        ...
        public static NDarray Relu(NDarray x, float alpha = 0.0f, float? max_value = null, float threshold = 0.0f)
        {
            ...
            {
                ["x"] = x, ["alpha"] = alpha, ["max_value"] = max_value, ["threshold"] = threshold
            };
        ...
        {
            var parameters = new Dictionary<string, object> {["x"] = x, ["alpha"] = alpha};
            return new NDarray(InvokeStaticMethod(caller, "elu", parameters));
        }

        /// <summary>
        ///     Scaled Exponential Linear Unit (SELU).
        ///     SELU is equal to: scale* elu(x, alpha), where alpha and scale are predefined constants.The values of alpha and
        ///     scale are chosen so that the mean and variance of the inputs are preserved between two consecutive layers as long
        ///     as the weights are initialized correctly(see lecun_normal initialization) and the number of inputs is "large
        ///     enough" (see references for more information).
        /// </summary>
        /// <param name="x">Input tensor.</param>
